...
def fetch_pools():
    while True:
        try:
            response = requests.get("https://server.duinocoin.com/getPool").json()
    ...
    try:
        ...
        while True:
            ...
            if UseLowerDiff:
                ...
                soc.send(
                    bytes(
                        "JOB," + str(username) + ",LOW," + str(mining_key),
                        encoding="utf8",
                    )
                )
            else:
                ...
                soc.send(
                    bytes(
                        "JOB," + str(username) + ",MEDIUM," + str(mining_key),
                        encoding="utf8",
                    )
                )
            ...
            temp_hash = None
            temp_data.append(base_hash.hexdigest())
            for result in range(100 * int(difficulty) + 1):
                ...
                if job[1] == ducos1:
                    temp_data.append(job[1])
                    temp_data.append(difficulty)
